...
const Signup = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // If account exists, redirect to login to link providers there
      if (result.error === "ACCOUNT_EXISTS" && result.email) {
        navigate("/login", {
          replace: true,
          state: {
            message: `An account already exists with ${result.email}. Please sign in to link your Google account.`,
            email: result.email,
